...
namespace NS_Education.Tools.BeingValidated
{
    public interface IBeingValidated<T>
    {
        ...
        /// <param name="onException">（可選）當驗證過程發生 Exception 時，執行的方法。未設定時，不做任何 catch。</param>
        /// <returns>此物件本身。</returns>
        IBeingValidated<T> Validate(Action<T> validation, Action<Exception> onException = null);
        ...
        /// <param name="onException">（可選）當驗證過程發生 Exception 時，執行的方法。未設定時，不做任何 catch。</param>
        /// <returns>此物件本身。</returns>
        Task<IBeingValidated<T>> ValidateAsync(Func<T, Task> validation, Action<Exception> onException = null);
        ...
        /// <returns>此物件本身。</returns>
        IBeingValidated<T> SkipIfAlreadyInvalid(bool setTo = true);
    }
}