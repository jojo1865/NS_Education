...
namespace NS_Education.Models.APIItems
{
    public abstract class BaseGetResponseRowWithCreUpd : IGetResponseRow
    {
        ...
        private async Task _SetInfoFromEntity<TEntity>(TEntity entity, PublicClass controller)
        {
            ...
            CreDate = entity.GetIfHasProperty<TEntity, DateTime?>(DbConstants.CreDate) is DateTime creDate
                ? creDate.ToFormattedStringDateTime()
                : null;
            ...
            UpdDate = entity.GetIfHasProperty<TEntity, DateTime?>(DbConstants.UpdDate) is DateTime updDate
                ? updDate.ToFormattedStringDateTime()
                : null;
