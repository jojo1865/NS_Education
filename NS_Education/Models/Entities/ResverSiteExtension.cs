...
namespace NS_Education.Models.Entities
{
    ...
    public static class ResverSiteExtension
    {
        ...
        {
            return rs.CalculatePriceByTimeSpan(timeSpans, rs.QuotedPrice);
        }

        /// <summary>
        /// 傳入一個預約場地，其所有預約時段與計算所需的參數，取得這個預約場地在每個時段的定價
        /// </summary>
        /// <param name="rs">預約場地</param>
        /// <param name="timeSpans">所有時段</param>
        /// <returns>
        ...
        /// 當 currentIndex 等於 maxCount - 1 （是最後一個時段時）：完整報價減掉其他所有時段的金額
        /// </returns>
        public static IEnumerable<decimal> GetFixedPriceByTimeSpan(this Resver_Site rs,
            IEnumerable<D_TimeSpan> timeSpans)
        {
            return rs.CalculatePriceByTimeSpan(timeSpans, rs.FixedPrice);
        }

        private static IEnumerable<decimal> CalculatePriceByTimeSpan(this Resver_Site rs,
            IEnumerable<D_TimeSpan> timeSpans, decimal wholePrice)
        {
            // 因為舊系統的場地價是依時段填
            // 但新系統只填單一個價格 for 預約場地
            // 所以在某些依時段把場地價格區分開來的舊系統既有報表，顯示報價時，就會有落差
            // 所以需要透過這個方法特殊計算
            // 雖然每行細項會有 (0 ~ 時段數) 塊的差異
            // 但最後總價會相同
            ...
            ICollection<decimal> results = timeSpans
                ...
                .Select(dts =>
                {
                    ...
                    decimal rawPrice = wholePrice * weight;
            ...
            decimal last = wholePrice - othersSum;
